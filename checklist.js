...
import { selection, impact, success } from "./appPrefs";
...
export default function Checklist() {
  ...
  // get localized dataset from i18n (changes when lang changes)
  ...
  // ensure selectedCatId stays valid after language/dataset switches
  useEffect(() => {
  ...
  const subProgress = useCallback(
    (sub) => {
      ...
      const dItems = items.filter((it) => !!checked[it.id]).length;
      const p = tItems ? Math.round((dItems / tItems) * 100) : 0;
      return { d: dItems, t: tItems, p };
    },
    [checked]
  );
        ...
        {currentCategory?.subcategories?.map((sub) => {
          ...
          return (
            <View key={sub.id} style={styles.subBlock}>
              ...
              {/* sub progress bar directly under header */}
