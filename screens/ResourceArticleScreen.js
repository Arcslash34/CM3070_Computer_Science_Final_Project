/**
 * screens/ResourceArticleScreen.js — Rich article reader with image zoom (presentational)
 *
 * Purpose
 * - Render localized safety/how-to articles with hero image, quick steps, detailed sections, news, and references.
 * - Support full-screen pinch/double-tap zoom for the hero image.
 *
 * ViewModel (vm) contract
 * - i18n/layout: t, insets
 * - data: article {
 *     title, imageKey|image, tags?, quick?, sections?|body?, links?, newsLinks?
 *   }
 * - nav: onBack()
 *
 * Key Behaviours
 * - Normalizes section shapes (arrays/objects) via `normalizeSections`.
 * - Optional blocks render only when present: tags, news, quick steps, sections/body, references.
 * - Opens links with `Linking.openURL`; hero supports full-screen zoom (pinch / double-tap).
 *
 * UX / Accessibility
 * - Back FAB over hero; large tap targets; readable bullets with adequate spacing.
 * - Numbers for quick steps; pills for tags; clear “References” and “News Articles” labels.
 *
 * Performance Notes
 * - Memoizes normalized sections and image source resolution.
 * - Avoids mounting zoom modal until requested.
 *
 * Fail-safes
 * - Falls back to “Guide” title and legacy `body` when `sections` absent.
 * - Ignores empty/invalid section entries; safe checks for arrays.
 */
...
export default function ResourceArticleScreen({ vm }) {
  ...
  const hasNews =
    Array.isArray(article?.newsLinks) && article.newsLinks.length > 0;
        ...
        {imageUri && (
          <View style={styles.heroWrap}>
            ...
            <TouchableOpacity
              ...
              accessibilityLabel={
                t("resourceArticle.back") !== "resourceArticle.back"
            ...
            {article.newsLinks.map((n, idx) => (
              ...
              >
                ...
                <View style={{ flex: 1, marginLeft: 8 }}>
                  <Text style={styles.newsRowTitle} numberOfLines={2}>
                    {n.label}
                  </Text>
                  {!!n.source && (
                    <Text style={styles.newsRowSource} numberOfLines={1}>
                      {n.source}
                    </Text>
                  )}
                </View>
